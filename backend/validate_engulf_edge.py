...
def run():
    ...
    hmax = g["high"].transform(lambda s: s.shift(1).rolling(3).max())
    lmin = g["low"].transform(lambda s: s.shift(1).rolling(3).min())
    df["engR3"] = (df["high"] >= hmax) & (df["low"] <= lmin)
    ...
    df["edge_in3"] = ((g_ae := ae.groupby(df["ticker"])).shift(1).fillna(0)
                      + g_ae.shift(2).fillna(0) + g_ae.shift(3).fillna(0)) > 0

    base = df["anybt"] & df["clean"] & df["engR3"]
    qual = base & (df["close"] >= 21) & (df["rsi_14"] < 45)     # the winning filter
    print(f"as_of {as_of} · engulf-3 base n={int(base.sum())} · edge-in-3 rate {df.loc[base,'edge_in3'].mean()*100:.0f}%\n")

    print("── engulf-3 (ANY bull-T) split by EDGE-signal in the 3 swept bars ──")
    print(_line("engulf3 ALL", base, df))
    print(_line("  + edge-in-3 ✓", base & df["edge_in3"], df))
    print(_line("  + NO edge-in-3", base & ~df["edge_in3"], df))

    print("\n── same, on the WINNING filter (≥$21 · RSI<45) ──")
    print(_line("engulf3+qual ALL", qual, df))
    print(_line("  + edge-in-3 ✓", qual & df["edge_in3"], df))
    print(_line("  + NO edge-in-3", qual & ~df["edge_in3"], df))

    print("\n── which Edge setup absorbed (engulf3+qual & that edge in prior 3) ──")
    aeq = qual
    for col in E_COLS:
        c = df[col].astype(float).groupby(df["ticker"])
        inp = (c.shift(1).fillna(0) + c.shift(2).fillna(0) + c.shift(3).fillna(0)) > 0
        m = aeq & inp
        if int(m.sum()) >= 40:
