...
def run_ultra_enrich_job(
    ...
    # Persist enriched Stage 2 results to DB (non-fatal)
    ...
    if enriched_rows:
        try:
            persist_ultra_scan_results(
                universe, tf, nasdaq_batch,
                rows=enriched_rows, last_scan=cached.get("last_scan") if cached else None,
                warnings=list(cached.get("warnings") or []) if cached else [],
                sources=dict(cached.get("sources") or {}) if cached else {},
            )
        except Exception as _exc:
            log.warning("ULTRA: Stage 2 DB persist skipped: %s", _exc)
...
def get_ultra_results(universe: str, tf: str, nasdaq_batch: str = "") -> dict:
    ...
    # Fall back to DB if in-memory cache is empty (e.g. after restart/deploy)
...
def get_ultra_latest_from_db(universe: str = "sp500", tf: str = "1d") -> dict:
    """
    Public API: metadata for the latest DB scan used by the dashboard bootstrap endpoint.
    Returns a dict with has_data, scan_run_id, status, finished_at, total_candidates, data_age_seconds.
    """
    try:
        from db import get_db
        nb = ""
        with get_db() as db:
            db.execute(
                """SELECT id, universe, tf, status, total_candidates, last_turbo_scan, finished_at, created_at
                   FROM ultra_scan_runs
                   WHERE universe=? AND tf=? AND nasdaq_batch=? AND is_latest=1 AND status='completed'
                   ORDER BY id DESC LIMIT 1""",
                (universe, tf, nb),
