...
SPEC = os.path.join(HERE, "FREE_FLIGHT_V2_SPEC.json")
REPORT = os.path.join(HERE, "FREE_FLIGHT_V2_RESULT.json")
...
def run(universe: str = "sp500", window: str = "mining") -> dict:
    ...
    arms = {
        "void_above (a)": tok.isin(vac),
        "same position, no void above": tok.isin(con),
        "void_above + bullish T": tok.isin(vac) & bull,
        "same position, no void + bullish T": tok.isin(con) & bull,
        "ALL BARS (base rate)": tok.notna(),
    }

    out = {"study": "FREE_FLIGHT_v2", "spec_hash": spec["spec_hash"], "universe": universe,
...
def verdict(res: dict) -> dict:
    ...
    spec = _spec()
    v = res["arms"]["void_above (a)"]["r10"]
    c = res["arms"]["same position, no void above"]["r10"]
    if not v or not c:
...
if __name__ == "__main__":
    ...
    for band, arms in r["by_price_bucket"].items():
        a = arms["void_above (a)"].get("r10")
        b = arms["same position, no void above"].get("r10")
        if a and b:
            print(f"    {band:<12}void {a['median']:>+7.2f} (n={a['n']:>6,})   "
