"""
validate_seq_t1t2t2.py — DEEP research on the 3-consecutive-bar T-sequences
  A = T1  → T2 → T2      B = T1G → T2 → T2
...
def run():
    ...
    A = (p2 == "T1") & (p1 == "T2") & (df["t"] == "T2")
    B = (p2 == "T1G") & (p1 == "T2") & (df["t"] == "T2")
    rsi = df["rsi_14"]
    # precompute every mask as a column (group once, reuse)
    M = {
        "A": A, "B": B, "Ac": A & clean, "Bc": B & clean,
        "T2_plain": df["t"] == "T2",
        # descriptor slices on the ENTRY bar, applied to A∪B clean (the family)
    }
    fam = (A | B) & clean
    M["FAM"] = fam
    ...
    print("trail25/60 · entry after last T2 · dv≥3M\n")
    print("── baselines ──")
    print(_line("T2 plain (any)", "T2_plain"))
    print("\n── the two sequences ──")
    print(_line("A: T1-T2-T2 (raw)", "A"))
    print(_line("A: T1-T2-T2 (clean)", "Ac"))
    print(_line("B: T1G-T2-T2 (raw)", "B"))
    print(_line("B: T1G-T2-T2 (clean)", "Bc"))
    print("\n── FAMILY (A∪B, clean) sliced by ENTRY-bar descriptors ──")
    ...
    print("  · vol / L:")
    for k in ["FAM_volB", "FAM_volVB", "FAM_L5", "FAM_L46", "FAM_L34", "FAM_Lnone"]:
        print(_line("   " + k.replace("FAM_", ""), k))
    print("  · price / body / gap:")
    for k in ["FAM_p_8_21", "FAM_p_21_89", "FAM_p_ge89", "FAM_bull", "FAM_bear", "FAM_gapup"]:
        print(_line("   " + k.replace("FAM_", ""), k))
    print("  · stacks:")
