...
Qualifying cells saved to backend/data/allpairs_all.csv (nothing lost again).
...
for a1,b1 in itertools.combinations(keys,2):
    ...
    qual.append(("BULL" if pm>0 else "BEAR",round(u-(base if pm>0 else lo_base),1),a1,b1,nn,round(u,1),round(pm,2),kk,tt,round(ptr,2) if not np.isnan(ptr) else None,round(pte,2) if not np.isnan(pte) else None))
...
Q.to_csv('/Users/sachoki/Desktop/sachoki-desktop/backend/data/allpairs_all.csv',index=False)
QB=Q[Q.dir=="BULL"].sort_values("ps",ascending=False)
QR=Q[Q.dir=="BEAR"].sort_values("ps")
Q2=Q
print(f"saved {len(Q2)} cells")
