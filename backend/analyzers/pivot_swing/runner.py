...
Usage:
    ...
    # Directory of CSVs (aggregated to ONE output set)
    python -m analyzers.pivot_swing.runner \
        --csv-dir /path/to/stock_stat_dir \
        --out /tmp/pivot_output
"""
import argparse
import glob
...
def main():
    ...
    parser.add_argument("--csv-dir", help="Directory of stock_stat_tz_wlnbb_*.csv files")
    ...
    if args.csv:
        ...
        csv_paths = sorted(glob.glob(os.path.join(args.csv_dir, "stock_stat_tz_wlnbb_*.csv")))
    else:
        parser.print_help()
        sys.exit(1)
    ...
    log.info("Processing %d CSV file(s) → aggregated output dir %s", len(csv_paths), args.out)
