    ...
    try:
        rows = conn.execute(
            f"""SELECT ticker, date, open, high, low, close, volume,
                      ...
                      -- ALL SIX raw measures, not the two that happened to get typed. The
                      -- first version listed phys_k_x and phys_r_raw only, so m/e/c/h_raw
                      -- were fully populated in the DB and arrived as empty columns in the
                      -- CSV export — the classification letters were there, the numbers
                      -- behind four of them were not, and an empty column reads as "this
                      -- was never measured" rather than "this was never sent".
                      phys_r_raw, phys_m_raw, phys_e_raw, phys_c_raw, phys_h_raw,
                      phys_k_x, phys_s_net, phys_e_release,
