...
Output: cluster profiles + members -> h1_russell_segments.csv
"""
import numpy as np
import pandas as pd
...
NQ = set(r[0] for r in c.execute("SELECT DISTINCT ticker FROM bars WHERE universe='russell2k'").fetchall())
c.close()
print(f"russell2k tickers: {len(NQ)}", flush=True)
...
F.round(3).to_csv("h1_russell_segments.csv")
print("\nsaved -> h1_russell_segments.csv", flush=True)
