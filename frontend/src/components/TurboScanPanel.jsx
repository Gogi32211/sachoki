...
const SIG_GROUPS = [
  ...
  // B signals (B1–B11 / ANY B) retired from the UI — still computed backend-side.
  { divider: true },
  // ── T/Z ───────────────────────────────────────────────────────────────
...
export default function TurboScanPanel({ onSelectTicker }) {
  ...
  const [swingTypeFilter, setSwingTypeFilter] = useState('')
  // 260523 v3.5 — PREBREAK + WYC additional filters
  const [prebreakTier, setPrebreakTier]   = useState('')
  const [pbLvbo,       setPbLvbo]         = useState(null)
        ...
        <div className="border-b border-white/[0.07] bg-md-surface-con/30">
          ...
          <div className="flex flex-wrap items-center gap-x-1 gap-y-1 px-3 py-1.5 border-t border-md-outline-var/30">
            ...
            <button onClick={() => setPbLvbo(pbLvbo ? null : true)}
              title="LRC → LVBO compression then bull breakout"
              ...
              <tr key={r.ticker}
                ...
                <td className="px-2 py-1">
                  <div className="flex flex-wrap gap-0.5">
                    ...
                    {/* B signals (B1–B11) retired from display — still computed backend-side */}
