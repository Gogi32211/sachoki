...
// ── Signal groups — flat list used for filter logic ───────────────────────────
const SIG_GROUPS = [
  ...
  { divider: true },
  { key: '_tz_bull',   label: 'T/Z↑',  cls: 'text-violet-300', custom: r => !!r.tz_sig },
  { key: '_tz_strong', label: 'T4/T6', cls: 'text-violet-200', custom: r => ['T4','T6','T1G','T2G'].includes(r.tz_sig) },
  { key: 'tz_bull_flip', label: 'TZ→3', cls: 'text-lime-300' },
  { key: 'tz_attempt',   label: 'TZ→2', cls: 'text-cyan-300' },
  { key: '_tz_weak', label: 'W', cls: 'text-yellow-300', custom: r => r.tz_weak_bull || r.tz_weak_bear },
  { divider: true },
  ...
  { divider: true },
  { key: '_rsi_os',  label: 'RSI≤35', cls: 'text-lime-300',    custom: r => (r.rsi || 100) <= 35 },
  { key: '_rsi_ob',  label: 'RSI≥70', cls: 'text-red-400',     custom: r => (r.rsi || 0) >= 70   },
  { key: '_yf_only', label: 'yf',     cls: 'text-orange-400',  custom: r => r.data_source === 'yfinance' },
  { divider: true },
  { key: '_cross2',  label: '⚡×2+', cls: 'text-yellow-300', custom: r => engineFamilies(r).size >= 2 },
  { key: '_cross3',  label: '⚡×3+', cls: 'text-lime-300',   custom: r => engineFamilies(r).size >= 3 },
  { key: '_cross4',  label: '⚡×4+', cls: 'text-lime-400',   custom: r => engineFamilies(r).size >= 4 },
  { key: '_early',   label: '[E]',   cls: 'text-sky-300',    custom: r => setupPhase(r) === 'Early'   },
]

// ── Signal families for the grouped filter UI ─────────────────────────────────
const SIG_FAMILY_TABS = [
  { id: 'all',   label: 'All' },
  {
    id: 'vabs', label: 'VABS',
    keys: ['best_sig','strong_sig','vol_spike_20x','vol_spike_10x','vol_spike_5x','vbo_up','abs_sig','climb_sig','load_sig','ns','sq','sc','nd'],
  },
  {
    id: 'tz', label: 'T/Z',
    keys: ['_tz_bull','_tz_strong','tz_bull_flip','tz_attempt','_tz_weak'],
  },
  {
    id: 'delta', label: 'Δ Delta',
    keys: ['d_blast_bull','d_surge_bull','d_strong_bull','d_absorb_bull','d_spring','d_div_bull','d_vd_div_bull','d_cd_bull','d_flip_bull','d_orange_bull','d_blast_bull_red','d_surge_bull_red','d_surge_bear_grn','d_blast_bear_grn','d_vd_div_bear'],
  },
  {
    id: 'lsig', label: 'L-Sig',
    keys: ['fri34','fri43','fri64','l34','l43','l64','l22','l555','only_l2l4','blue','cci_ready','cci_0_retest','cci_blue_turn','bo_up','bo_dn','bx_up','bx_dn','be_up','be_dn','fuchsia_rh','fuchsia_rl','pre_pump','x2g_wick','x2_wick','x1g_wick','x1_wick','x3_wick','wick_bull'],
  },
  {
    id: 'combo', label: 'Combo',
    keys: ['buy_2809','rocket','sig3g','rtv','hilo_buy','atr_brk','bb_brk','va','bias_up','um_2809','svs_2809','conso_2809','cd','ca','cw','b1','b2','b3','b4','b5','b6','b7','b8','b9','b10','b11','g1','g2','g4','g6','g11','seq_bcont','best_long','fbo_bull','fbo_bear','eb_bull','eb_bear','bf_buy','bf_sell','ultra_3up','sig_l88','sig_260308'],
  },
  {
    id: 'ema', label: 'EMA',
    keys: ['preup66','preup55','preup89','preup3','preup2','preup50','predn66','predn55','predn89','predn3','predn2','predn50'],
  },
  {
    id: 'ctx', label: 'Context',
    keys: ['rs_strong','rs','_rsi_os','_rsi_ob','_yf_only','_cross2','_cross3','_cross4','_early'],
  },
]

// ── T/Z display constants ─────────────────────────────────────────────────────
...
// ── Mini chart popup ──────────────────────────────────────────────────────────
function MiniChartPopup({ row, tf, pos }) {
  ...
  const CHART_W = 780, CHART_H = 380

  useEffect(() => {
    if (!containerRef.current) return
    const isIntraday = ['30m','15m','1h','4h'].includes(tf)
    const chart = createChart(containerRef.current, {
      layout: { background: { color: '#0f1117' }, textColor: '#9fb3cc' },
      grid: { vertLines: { color: '#2d3a50' }, horzLines: { color: '#2d3a50' } },
      crosshair: { mode: 1 },
      rightPriceScale: { borderColor: '#374357' },
      timeScale: { borderColor: '#374357', timeVisible: isIntraday },
    ...
    api.signals(row.ticker, tf, 80)
      .then(rows => {
        ...
        if (bars.length) { series.setData(bars); chart.timeScale().fitContent() }
        setLoading(false)
      })
  ...
  const POPUP_W = 820, POPUP_H = 520
  const vw = window.innerWidth, vh = window.innerHeight
  let left = pos.x + 16; if (left + POPUP_W > vw - 8) left = pos.x - POPUP_W - 8
  let top  = pos.y - 80; if (top + POPUP_H > vh - 8) top = vh - POPUP_H - 8; if (top < 8) top = 8
  const chg = row.change_pct ?? 0

  return (
    <div className="fixed z-50 bg-gray-900 border border-gray-700 rounded-lg shadow-2xl text-xs text-gray-100 pointer-events-none"
      ...
      <div className="flex items-center justify-between px-4 py-2.5 border-b border-gray-700">
        <div className="flex items-center gap-3 min-w-0">
          ...
          {info?.name && info.name !== row.ticker && <span className="text-gray-300 text-sm truncate">{info.name}</span>}
          {info?.sector && <span className="text-gray-500 text-xs shrink-0 bg-gray-800 px-1.5 py-0.5 rounded">{info.sector}</span>}
        </div>
      ...
      <div className="relative">
        ...
        {loading && <div className="absolute inset-0 flex items-center justify-center bg-gray-900/70 text-gray-500">Loading…</div>}
      </div>
      <div className="px-4 py-2 text-gray-400 text-xs truncate border-t border-gray-800">{scoreReason(row)}</div>
    </div>
  )
}
...
function StarBtn({ ticker, tf, onToggle }) {
  ...
  return (
    <button title={saved ? 'Remove from watchlist' : 'Save to watchlist'}
      className={`text-sm transition-colors ${saved ? 'text-yellow-400' : 'text-gray-700 hover:text-yellow-400'}`}
      onClick={e => { e.stopPropagation(); onToggle?.(); setSaved(s => !s) }}>
      ★
    </button>
  )
}
...
export default function TurboScanPanel({ onSelectTicker }) {
  ...
  const loadFromCache = useCallback((tf, uni) => {
    ...
    if (!cached?.results?.length && uni !== 'all_us') cached = _tsGet(tf, 'all_us')
    if (cached?.results?.length) {
      setAllResults(cached.results); setLastScan(cached.lastScan || null)
    } else {
      setAllResults([]); setLastScan(null)
    }
  }, [])
  ...
  const fetchFreshResults = useCallback((tf, uni) => {
    api.turboScan(10000, 0, 'all', tf, uni, {})
      .then(d => {
        ...
        if (results.length > 0) { turboCacheSet(tf, uni, results, ls); setAllResults(results); setLastScan(ls || null) }
      })
  ...
  const results = useMemo(() => {
    const filtered = allResults.filter(r => {
      ...
      if (selSigs.size > 0) {
        if (!r._ages && r.sig_ages) { try { r._ages = JSON.parse(r.sig_ages) } catch { r._ages = {} } }
        const ages = r._ages || {}
  ...
  const SortTh = ({ col, children, cls = '' }) => (
    <th className={`px-2 py-1.5 font-medium cursor-pointer select-none hover:text-white transition-colors ${cls}`}
      onClick={() => toggleSort(col)}>
      {children}{sortBy === col ? (sortDir === 'desc' ? ' ↓' : ' ↑') : ''}
    </th>
  )

  const toggleSig = key => setSelSigs(prev => { const n = new Set(prev); n.has(key) ? n.delete(key) : n.add(key); return n })
  const togglePicked = (ticker, e) => {
    e.stopPropagation()
    setPickedTickers(prev => { const n = new Set(prev); n.has(ticker) ? n.delete(ticker) : n.add(ticker); return n })
  }

  const exportTickers = () => {
    const src = pickedTickers.size > 0 ? results.filter(r => pickedTickers.has(r.ticker)) : results
    const text = src.map(r => r.ticker).join('\n')
    const blob = new Blob([text], { type: 'text/plain' })
    const url  = URL.createObjectURL(blob)
    const a    = document.createElement('a')
    a.href = url; a.download = `sachoki-${new Date().toISOString().slice(0,10)}.txt`
  ...
  const handleRowLeave = useCallback(() => { clearTimeout(hoverTimer.current); setHoverPopup(null) }, [])

  useEffect(() => () => { if (pollIvRef.current) clearInterval(pollIvRef.current); clearTimeout(hoverTimer.current) }, [])

  const _stopPoll = () => { if (pollIvRef.current) { clearInterval(pollIvRef.current); pollIvRef.current = null } }

  const _poll = () => {
    _stopPoll()
    const activeTf = localTf, uni = universe
  ...
  const scan = () => {
    ...
    api.turboScanTrigger(localTf, universe, lookbackN, partialDay, volMin)
      ...
      .catch(e => {
        ...
        if (msg.includes('409') || msg.toLowerCase().includes('already running')) setError('__stuck__')
        else setError(msg)
      })
  }

  // ── Signals to show in filter for current family tab ──────────────────────
  const visibleSigs = useMemo(() => {
    if (sigFamily === 'all') return SIG_GROUPS
    const fam = SIG_FAMILY_TABS.find(f => f.id === sigFamily)
    if (!fam) return SIG_GROUPS
    return SIG_GROUPS.filter(s => !s.divider && fam.keys.includes(s.key))
  }, [sigFamily])
    ...
    <div className="flex flex-col h-full bg-gray-950 text-gray-100 text-xs" onMouseLeave={handleRowLeave}>
      ...
      <div className="flex flex-wrap items-center gap-2 px-3 py-2 border-b border-gray-800">
        ...
        <div className="flex gap-0.5 border border-gray-700 rounded p-0.5">
          {TF_OPTS.map(t => (
            <button key={t}
              onClick={() => { setLocalTf(t); setLastScan(null); try { localStorage.setItem('sachoki_turbo_tf', t) } catch {} }}
              title={tfCached[t] ? `${t.toUpperCase()} — cached` : `${t.toUpperCase()} — scan first`}
              ...
              {t.toUpperCase()}
              {tfCached[t] && <span className="absolute -top-0.5 -right-0.5 w-1.5 h-1.5 rounded-full bg-green-400" />}
            </button>
          ))}
        </div>
        ...
        <button onClick={scan} disabled={scanning}
          className={`px-3 py-1 rounded text-xs font-semibold transition-colors
            ${scanning ? 'bg-gray-700 text-gray-400 cursor-not-allowed' : 'bg-violet-600 hover:bg-violet-500 text-white'}`}>
          {scanning ? <span className="animate-pulse">⚡ Scanning…</span> : '⚡ TURBO'}
        </button>

        {/* Preview */}
        <button onClick={() => setPartialDay(p => !p)}
          title="Include today's in-progress daily bar"
          className={`px-2.5 py-1 rounded text-xs font-medium transition-colors border
            ${partialDay ? 'border-amber-400 text-amber-300 bg-amber-900/30' : 'border-gray-700 text-gray-500 hover:text-gray-300 hover:border-gray-500'}`}>
          ~Preview
        </button>
        ...
        <button onClick={exportTickers} disabled={results.length === 0}
          className={`px-2.5 py-1 rounded text-xs font-medium transition-colors border
            ${exported ? 'border-lime-500 text-lime-300 bg-lime-900/30'
              : results.length === 0 ? 'border-gray-700 text-gray-600 cursor-not-allowed'
              : pickedTickers.size > 0 ? 'border-yellow-500 text-yellow-300 bg-yellow-900/20 hover:border-yellow-400'
              : 'border-gray-600 text-gray-300 hover:border-gray-400 hover:text-white'}`}>
          {exported ? '✓ Copied' : pickedTickers.size > 0 ? `⬇ Export (${pickedTickers.size})` : '⬇ Export'}
        </button>
        ...
        <div className="flex items-center gap-0.5 ml-1">
          ...
          {[
            ...
            return (
              <button key={label} onClick={() => { setVolMin(min); setVolMax(max) }}
                className={`px-2 py-0.5 rounded text-xs transition-colors
      ...
      {/* ── Row 2: Signal filter — family tabs + chips ── */}
      <div className="flex flex-col border-b border-gray-800 bg-gray-900/30">
        ...
        <div className="flex items-center gap-1 px-3 pt-1.5 pb-1">
          <span className="text-gray-500 text-xs mr-1 shrink-0">SIG</span>
          {SIG_FAMILY_TABS.map(fam => (
            <button key={fam.id}
              onClick={() => setSigFamily(fam.id)}
              className={`px-2.5 py-0.5 rounded text-xs transition-colors font-medium
                ${sigFamily === fam.id ? 'bg-blue-600 text-white' : 'bg-gray-800 text-gray-500 hover:text-gray-300'}`}>
              {fam.label}
            </button>
          ))}
          {selSigs.size > 0 && (
            <button onClick={() => setSelSigs(new Set())}
              className="ml-auto px-2 py-0.5 rounded text-xs bg-red-900/40 text-red-400 hover:bg-red-900/60 shrink-0">
              ✕ clear {selSigs.size}
            </button>
          )}
        </div>

        {/* Signal chips for active family */}
              ...
              : (
                <button key={s.key} onClick={() => toggleSig(s.key)}
                  className={`px-2 py-0.5 rounded text-xs shrink-0 transition-colors
                    ${selSigs.has(s.key) ? `${s.cls} bg-gray-700 font-semibold ring-1 ring-current/30` : 'bg-gray-800 text-gray-500 hover:text-white'}`}>
                  {s.label}
                </button>
              )
          )}
        ...
        <div className="px-4 py-1.5 text-red-400 border-b border-gray-800 flex items-center gap-3">
          {error === '__stuck__'
            ? <>
                ...
                <button onClick={() => api.turboScanReset().then(() => setError(null))}
          ...
          <tbody>
            {results.map(r => (
              <tr key={r.ticker}
                ...
                <td className="px-2 py-1.5 w-5" onClick={e => e.stopPropagation()}>
                  <input type="checkbox" className="accent-indigo-500 cursor-pointer"
                ...
                {/* Ticker */}
                <td className="px-2 py-1.5 font-mono font-semibold text-blue-300">
                  <div className="flex items-center gap-1">
                    <span>{r.ticker}</span>
                    {r.vol_bucket && <span className="text-gray-600 font-normal">{r.vol_bucket}</span>}
                    {r.data_source === 'yfinance' && <span className="text-[8px] text-orange-400/60 font-normal align-top">yf</span>}
                    <a href={`https://www.tradingview.com/chart/?symbol=${r.ticker}`} target="_blank" rel="noopener noreferrer"
                      onClick={e => e.stopPropagation()}
                      className="text-gray-600 hover:text-blue-400 transition-colors text-[10px] leading-none">↗</a>
                  </div>
                </td>

                {/* Score */}
                <td className="px-2 py-1.5 text-center" title={scoreReason(r)}>
                  {(() => {
                    const sc = r[effectiveScoreCol] ?? r.turbo_score ?? 0
                    return (
                      <>
                        <div className={`font-mono font-bold text-sm ${scoreColor(sc)}`}>{fmt(sc, 1)}</div>
                        <div className="text-[9px] text-gray-600 leading-tight mt-0.5 max-w-[72px] truncate">{scoreReason(r)}</div>
                      </>
                    )
                  })()}
                </td>
                ...
                <td className="px-2 py-1.5 text-center font-mono font-semibold">
                  {r.tz_sig
                    ? <span className={TZ_STRONG.has(r.tz_sig) ? 'text-lime-300' : TZ_BEAR.has(r.tz_sig) ? 'text-red-400' : 'text-blue-300'}>
                        {r.tz_sig}
                      </span>
                ...
                {/* RSI */}
                <td className={`px-2 py-1.5 text-center font-mono text-xs ${r.rsi >= 70 ? 'text-red-400' : r.rsi <= 30 ? 'text-lime-400' : 'text-gray-400'}`}>
                  {r.rsi != null ? fmt(r.rsi, 0) : '—'}
                </td>

                {/* CCI */}
                <td className={`px-2 py-1.5 text-center font-mono text-xs ${r.cci >= 100 ? 'text-lime-400' : r.cci <= -100 ? 'text-red-400' : 'text-gray-400'}`}>
                  {r.cci != null ? fmt(r.cci, 0) : '—'}
                </td>

                {/* Price */}
                <td className="px-2 py-1.5 text-right font-mono text-gray-200">${fmt(r.last_price)}</td>

                {/* Change % */}
                <td className={`px-2 py-1.5 text-right font-mono ${r.change_pct >= 0 ? 'text-lime-400' : 'text-red-400'}`}>
                  {r.change_pct >= 0 ? '+' : ''}{fmt(r.change_pct)}%
                  {r.avg_vol > 0 && (
                    <span className="ml-1 text-gray-600 font-normal text-xs">
                      {r.avg_vol >= 1_000_000 ? `${(r.avg_vol/1_000_000).toFixed(1)}M` : r.avg_vol >= 1_000 ? `${Math.round(r.avg_vol/1_000)}K` : Math.round(r.avg_vol)}
                    </span>
                  )}
                </td>
              </tr>
