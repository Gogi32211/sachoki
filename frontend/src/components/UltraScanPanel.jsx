        ...
        <table className="w-full border-collapse">
          <thead className="sticky top-0 bg-gray-900 z-10 text-gray-500 text-left">
            <tr>
              ...
              {/* ── ULTRA enrichment columns (read-only, additive) ── */}
          ...
          <tbody>
            {results.map(_raw => {
              ...
              <tr key={r.ticker}
                ...
                {/* ── ULTRA enrichment cells (read-only, additive) ─────────── */}

                {/* TZ/WLNBB */}
                <td className="px-2 py-1 whitespace-nowrap">
                  {r.tz_wlnbb ? (
                    <div className="flex flex-wrap gap-0.5">
                      {r.tz_wlnbb.t_signal     && <Badge label={`T:${r.tz_wlnbb.t_signal}`}     cls="bg-emerald-900/50 text-emerald-200" />}
                      {r.tz_wlnbb.z_signal     && <Badge label={`Z:${r.tz_wlnbb.z_signal}`}     cls="bg-blue-900/50 text-blue-200" />}
                ...
                <td className="px-2 py-1 whitespace-nowrap">
                  {r.tz_intel ? (
                    <div className="flex flex-wrap items-center gap-0.5"
                         title={[
                           r.tz_intel.matched_status ? `matched: ${r.tz_intel.matched_status}` : null,
                           r.tz_intel.matched_med10d_pct  != null ? `med10d: ${r.tz_intel.matched_med10d_pct}` : null,
                           r.tz_intel.matched_fail10d_pct != null ? `fail10d: ${r.tz_intel.matched_fail10d_pct}` : null,
                ...
                <td className="px-2 py-1 text-center whitespace-nowrap">
                  {r.abr && r.abr.category ? (
                    <span title={[
                      ...
                      r.abr.med10d_pct  != null ? `med10d: ${r.abr.med10d_pct}` : null,
                      r.abr.fail10d_pct != null ? `fail10d: ${r.abr.fail10d_pct}` : null,
                      r.abr.conflict_flag      ? 'CONFLICT' : null,
                      r.abr.confirmation_flag  ? 'CONFIRM' : null,
                    ].filter(Boolean).join('\n')}
                          className={`text-xs px-1.5 py-0.5 rounded border font-medium
                            ${r.abr.category === 'A'  ? 'bg-emerald-900/60 text-emerald-200 border-emerald-700/60'
